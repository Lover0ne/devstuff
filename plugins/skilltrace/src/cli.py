    ...
    deny_response = json.dumps({
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": (
                ...
                "If subagents or workflow agents were used for that task, include what EACH agent did and what files it modified. "
...
def cmd_finalize(hook_data: dict) -> dict:
    ...
    return {"additionalContext": (
        ...
        "If subagents or workflow agents were used, include what EACH agent did and what files it modified. "
        "EXCLUDE any skilltrace commands or skill management operations from the summary. "
        f"2) transcript_path={tp} "
        f"3) project_dir={cwd} "
        "4) 'Follow the skilltracer agent procedure from the skilltrace plugin.'"
